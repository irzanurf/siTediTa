...
result_judul = result_judul.replace('pengabdian :','')
...
result_lama_pengabdian = result_lama_pengabdian.replace('hart','hari').replace('> han','1 hari')
...
if index < 100:
    ...
    for x in range(len(text_arrays)-1-index):
        ...
        if 'Lama Pengabdian' in text_arrays[index]:#pengabdian5.pdf
            start_anggota_mahasiswa_index = index
            break
        print("nssssssssssssssssssssssssssss",result_anggota_mahasiswa_s)
        result_anggota_mahasiswa_s.append(''.join(i for i in text_arrays[index].replace(',','').replace('Namamahasiswa _:','').replace('Nama Anggota','').replace('Nama Mahasiswa _:','').replace('Nama Mahasiswa :','').replace('Mahasiswa terlibat','').replace('.','').replace('NIM','')  if not i.isdigit()))
        result_nim_mahasiswa_s.append(''.join(i for i in text_arrays[index] if i.isdigit()))
       

        index+=1
print(result_anggota_mahasiswa_s)
