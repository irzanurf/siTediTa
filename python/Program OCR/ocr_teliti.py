...
def cari_indeks_dosen(dosen2) :
    ...
    for i in range(len(dosen2)):
        ...
        c=c.replace("Dipl.","").replace("S.Si.","").replace("M.Ing.","").replace("MPS","").replace("Eng.","").replace("I.","").replace("T.","").replace("Dipl.Ing.","")
        c=c.replace("CES","").replace("DEA","").replace("a  Priyo Nugroho P  M","Priyo Nugroho Parmantoro").replace("L.M.","").replace("MAsc.","").replace("M.T","").replace("Dra.","").replace("Dipl.GE","").replace("S.T","")
        dosen_raw2.append(c)
    for i in range(len(dosen_raw2)):
        ...
        if len(lo)==1:
            ...
            continue
    i=0
    print("dosenrawwwwwwwwwwwwwwwwwwwwwwww",dosen_raw2)
    print(dosen_raw3)
    indeks_dosen=[]
...
for text_array in text_arrays:
    ...
    if 'Judul' in text_array:
        ...
        break
    if 'TAHUN ANGGARAN' in text_array:
        start_judul_index = index+1
        break
    if 'Sagan?' in text_array:
        start_judul_index = index+1
        break
    index+=1
...
if index !=0:
    for x in range(10):
        ...
        if 'Nama Mitra' in text_arrays[index]:
            ...
            break
        if 'TIM PENGUSUL' in text_arrays[index]:
            start_judul_index = index
            index_ketua_pelaksana = index+1
            break
...
result_abstrak = ""
if "BAB I" not in text_arrays[start_abstrak_index+1]:
...
if result_sumber_dana_array !="":
    ...
    if r_mentah_biaya == '':
        ...
        if len(result_biaya_array) ==3 :
                ...
                r_mentah_biaya = result_biaya_array[0]+"."+result_biaya_array[1]+"."+result_biaya_array[2]
        else:
...
print("waktu",start_lama_penelitian_index)
...
if index!=0:
    r_mentah_lama_penelitian2=re.sub('[^0-9]','', r_mentah_lama_penelitian);print(r_mentah_lama_penelitian2)
    print("mentah2",r_mentah_lama_penelitian)
    if int(r_mentah_lama_penelitian2) > 12 :
        r_mentah_lama_penelitian=r_mentah_lama_penelitian.replace("2",":").replace("+",":")
    else:
        r_mentah_lama_penelitian=r_mentah_lama_penelitian.replace("+",":")
...
for text_array in text_arrays:
    ...
    if 'Anggota Penelitian' in text_array:
        ...
        d_type = 3
        break
    if 'Anggota Peneliti' in text_array:
        start_anggota_dosen_index = index+1
        d_type = 4
    index+=1
...
if index_ketua_pelaksana > 0:
    if 'Nama' in text_arrays[index_ketua_pelaksana+1]:
        result_anggota_dosen_s.append(text_arrays[index_ketua_pelaksana+1].replace('Nama',''))
if d_type==1:
...
if d_type==5:
    ...
    for index in range(index,100):
        print("proses dosen",index)
        if 'NIM' in text_arrays[index]:
           break
        if 'DEPARTEMEN TEKNIK' in text_arrays[index]:
           break
        if '21050120410012' in text_arrays[index]:
           break
        print("proses dosen",result_anggota_dosen_s)
        result_anggota_dosen_s.append(''.join(i for i in text_arrays[index].replace('(NIP','').replace(')','') if not i.isdigit()))
        index+=1
...
for text_array in text_arrays:
    ...
    if 'Anggota Mahasiswa' in text_array:
        ...
        break
    if '_198105202003121002' in text_array:
        start_anggota_mahasiswa_index = index+1
        break
    if index == 100:
        break
    index+=1
if start_anggota_mahasiswa_index==0:
    index = 0
    for text_array in text_arrays:
         if '196204281990012001' in text_array:
                start_anggota_mahasiswa_index = index+1
                break
         index+=1
...
result_anggota_mahasiswa_s3 = []
result_nim_mahasiswa_s2 = []
result_anggota_mahasiswa = ""

if index < 100 and start_anggota_mahasiswa_index != 0 :
    ...
    for x in range(len(text_arrays)-1-index):
        ...
        if 'DEPARTEMEN TEKNIK' in text_arrays[index]:
            start_anggota_mahasiswa_index = index
            break
        if 'FAKULTAS TEKNIK' in text_arrays[index]:
            start_anggota_mahasiswa_index = index
            break
        result_anggota_mahasiswa_s2.append(''.join(i for i in text_arrays[index].replace('Anggota Mahasiswa','').replace('Anggota Mahasiswa :','').replace('Anggota Mahasiswa >','').replace('Nama Anggota','').replace('Mahasiswa terlibat','').replace('NIM','') if not i.isdigit()))
...
for i in range(len(result_anggota_mahasiswa_s2)):
        if result_anggota_mahasiswa_s2[b]!="":
            result_anggota_mahasiswa_s3.append(result_anggota_mahasiswa_s2[b].replace('= ( )','').replace('( )','').strip() )
        
        b+=1
print("mmmmmmmmmm",result_anggota_mahasiswa_s3)
b=0
for i in range(len(result_anggota_mahasiswa_s3)):
        if len(result_anggota_mahasiswa_s3[b])>5:
            result_anggota_mahasiswa_s.append(result_anggota_mahasiswa_s3[b].replace('= ( )','').replace('( )','').replace('d','').replace('b','') )
        b+=1
b=0
for i in range(len(result_nim_mahasiswa_s2)):
        if result_nim_mahasiswa_s2[b]!="" and len(result_nim_mahasiswa_s2[b])>8:
...
for mhs in result_anggota_mahasiswa_s :
    mhss1.append(mhs.replace(",","").replace(".","").replace(":","").replace("()","").replace("M ","M. ").strip());a+=1
