...
def cari_indeks_dosen(dosen2) :
    ...
    for i in range(len(dosen2)):
        ...
        c=c.replace("Dipl.","").replace("S.Si.","").replace("M.Ing.","").replace("MPS","").replace("Eng.","").replace("I.","").replace("T.","").replace("Dipl.Ing.","")
        c=c.replace("CES","").replace("DEA","").replace("L.M.","").replace("MAsc.","").replace("M.T","").replace("Dra.","").replace("Dipl.GE","").replace("S.T","").replace("ST","").replace("MT","")
        dosen_raw2.append(c)
    for i in range(len(dosen_raw2)):
        #print("raw dosennnnn",dosen_raw2[i])
        lo=difflib.get_close_matches(dosen_raw2[i], dosen_raw1, 1)
        #print("raw dosennnnn hasil",lo)
        if len(lo)==1:
...
if a!=2:
    index = 0
    for text_array in text_arrays:
        if 'SE MAR Ane' in text_array:
            start_judul_index = index+1
            break
        index+=1
...
if index!=0:
    for x in range(len(text_arrays)-1-index):
        if 'Ketua Pelaksana' in text_arrays[index]:
            ...
            break
        elif 'Nama Mitra' in text_arrays[index]:
            start_judul_index = index
            index_ketua_pelaksana = index+1
            break
        elif 'Ketua Tim'in text_arrays[index]:
            start_judul_index = index
            index_ketua_pelaksana = index+1
            break
        elif 'Ketua'in text_arrays[index]:
            start_judul_index = index
            index_ketua_pelaksana = index+1
            break
        elif 'Luaran Pengabdian' in text_arrays[index]:
                start_judul_index = index
                index_ketua_pelaksana = index+1
                break
        elif 'TIM PENGUSUL' in text_arrays[index]:
                start_judul_index = index
                index_ketua_pelaksana = index+1
                break
...
if "BAB 1" in text_arrays[start_abstrak_index+1]:
    index=0
    for text_array in text_arrays:
    # print(text_array)
        if '\x0c1' in text_array:
            start_abstrak_index = index+1
            break
        index+=1
...
result_biaya=result_biaya.replace("-","").replace("9.  ","").strip()
#######################################################
...
if start_lama_pengabdian_index!=0:
    ...
    result_lama_pengabdian = result_lama_pengabdian.replace('hart','hari').replace('> han','1 hari').replace('5.','').replace('8.','')
...
for text_array in text_arrays:
    ...
    if 'Lokasi Pengabdian' in text_array:
        ...
        if '4' in text_arrays[index]:
           start_lokasi_index = index
        else:
           start_lokasi_index = index+1
        break
    index+=1
print("lokasiiii",text_arrays[37])
...
if start_lokasi_index !=0 :
    for x in range(10):
        ...
        if ab!=0 :
            result_lokasi += ', '
        result_lokasi += text_arrays[index].replace('n,','').replace('o.','').replace('p.','').replace('a,','').replace('b,','').replace('c,','').replace('a.','').replace('b.','').replace('c.','').replace('Propmsi :','').replace('Kelurahan/Kecamatan : ','').replace('Desa/Kecamatan : ','').replace('Kabupaten/Kota : ','').replace('Propinsi : ','').replace('Kabupaten','').replace('h','').replace('Kecamatan','').replace('Provinsi','').replace('Kota :','').replace('Kota','').replace('Tenga','Tengah').replace('nareswan','nareswari').replace('Juwimng','Juwiring').replace('4, Lokasi Pengabdian :','').strip()
...
if index_ketua_pelaksana > 0:
    if 'Nama' in text_arrays[index_ketua_pelaksana+1]:
        ketua_pelaksana.append(text_arrays[index_ketua_pelaksana+1].replace('Nama',''))
if d_type==1:
    index = start_anggota_dosen_index
    for x in range(100):
        if 'Anggota' in text_arrays[index]:
            pass
        else:
            start_anggota_dosen_index = index
            break
...
if d_type==3:
    ...
    index = start_anggota_dosen_index
    print(len(text_arrays));print(len(text_arrays)-1-index)
    for x in range(len(text_arrays)-1-index):
        print("alllllllllllllllllljjjjjlllll",result_anggota_dosen_s)
        if 'Anggota mahasiswa' in text_arrays[index]:
            break
        if 'Mahasiswa' in text_arrays[index]:
            break
        result_anggota_dosen_s.append(''.join(i for i in text_arrays[index].replace('NIP.','') if not i.isdigit()));print(result_anggota_dosen_s)
        index+=1

print("alllllllllllllllllllllllllllllllllllllll",result_anggota_dosen_s)
...
for text_array in text_arrays:
    ...
    if 'Nama Mahasiswa' in text_array:
        ...
        if 'Mahasiswa terlibat' in text_array[start_anggota_mahasiswa_index+1]:
    ...
    if index < 100:
        ...
        for x in range(len(text_arrays)-1-index):
            ...
            if 'Lokasi Pengabdian' in text_arrays[index]:#pengabdian5.pdf
                start_anggota_mahasiswa_index = index
                break
            if 'DEPARTEMEN' in text_arrays[index]:#pengabdian5.pdf
                start_anggota_mahasiswa_index = index
                break
            if '[aResam [seam' in text_arrays[index]:#pengabdian5.pdf
                start_anggota_mahasiswa_index = index
                break
            result_anggota_mahasiswa_s2.append(''.join(i for i in text_arrays[index].replace(',','').replace(':','').replace('_','').replace('Namamahasiswa _:','').replace('Nama Anggota','').replace('Nama Mahasiswa _:','').replace('Nama Mahasiswa :','').replace('Nama Mahasiswa ','').replace('Namamahasiswa','').replace('Mahasiswa terlibat','').replace('.','').replace('NIM','').replace('=','')  if not i.isdigit()))
...
print("Anggota Dosen : ")
dsns = result_anggota_dosen_s

print("\n")

nip_ketua_pelaksana = ambil_nip_dosen(cari_indeks_dosen(ketua_pelaksana))
ketua_pelaksana=ambil_nama_dosen(cari_indeks_dosen(ketua_pelaksana))
...
data_set = {
    ...
    "kecepatan": kecepatan,
    "ketua": ketua_pelaksana,
    "nip_ketua": nip_ketua_pelaksana,
    }
print(data_set)
